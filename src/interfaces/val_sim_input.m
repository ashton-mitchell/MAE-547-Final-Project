function val_sim_input(input)
    ...
    assert(length(input.robot.joint_min) == N, 'joint_min mismatch');
    assert(length(input.robot.joint_max) == N, 'joint_max mismatch');
    ...
    if strcmp(input.control.mode, 'Impedance')
        assert(length(input.control.Md) == N, 'Md mismatch');
        assert(length(input.control.Bd) == N, 'Bd mismatch');
        assert(length(input.control.Kd) == N, 'Kd mismatch');
    end
